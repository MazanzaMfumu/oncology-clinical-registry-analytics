...
that reproduces selected components of an oncology clinical
data-management workflow.

It is not an official hospital registry, electronic health-record
system, clinical trial database or national cancer-registry submission
system.
...
* official registry reporting;
* re-identification of individuals;
* replacement of medical or institutional data validation.
...
The dataset source, version, stated licence, DOI and known provenance
limitations are documented in:

[`data_source_register.md`](data_source_register.md)

Although Kaggle displays a CC BY 4.0 licence, the repository does not
redistribute the original row-level CSV. This conservative decision
reflects the need to distinguish between the licence displayed by an
intermediary platform and the complete provenance and redistribution
conditions of the underlying data.
...
* aggregated reports;
* aggregated figures;
* synthetic demonstration data where required.

### Excluded files

The following files are processed locally and excluded from Git:

* the original row-level CSV;
* prepared row-level datasets;
* validated row-level datasets;
...
The exclusions are implemented through `.gitignore`.

The absence of a file from the current working tree does not, by itself,
prove that it was never committed historically. Repository history
should therefore be reviewed before public release.

### Identifiers

The source dataset used by this project is not intentionally combined
with direct hospital identifiers such as:
...
* hospital medical-record numbers.

The variable `case_id` is generated sequentially by the project code for
technical traceability.

It is not:

* an original patient identifier;
* a hospital identifier;
* a medical-record number;
* a pseudonym supplied by a healthcare institution.

### Data minimisation and public outputs

Publicly distributed outputs should contain only the information needed
to demonstrate the analytical workflow.

The repository therefore prioritises:

* aggregated quality indicators;
* aggregated stage distributions;
* aggregated survival summaries;
* statistical model outputs;
* non-identifying figures;
* fully synthetic dashboard demonstrations.

The Streamlit application uses the local SQLite database only in the
local development environment. When that database is unavailable, the
application uses fully synthetic data generated by the project code.

Synthetic data are used for software demonstration only. They must not
be presented as observed clinical findings.

### Missing and anomalous data

Missing clinical values are not silently replaced during preparation.

Values that cannot be converted to the expected numerical format become
missing and are passed to the validation stage.

Records affected by a technical quality issue are flagged for review
rather than automatically deleted.

A `needs_review` status does not establish that the underlying clinical
information is false. It indicates that at least one technical rule
requires verification.

This approach preserves:

* source traceability;
* transparency of quality decisions;
* separation between technical validation and clinical adjudication.

### Statistical and clinical interpretation

The analyses describe the processed portfolio dataset only.

Observed associations, Kaplan-Meier estimates, log-rank tests and Cox
model outputs must not be interpreted as:

* causal effects;
* official SEER statistics;
* population-level surveillance estimates;
* individual survival predictions;
* treatment recommendations;
* evidence suitable for direct clinical use.

Clinical interpretation would require additional validation of the
source, coding definitions, population selection, missing-data
mechanisms, confounding, model assumptions and institutional context.

### Fairness and responsible communication

Variables describing demographic or clinical groups must be interpreted
with caution.
...
The project does not use demographic categories to make automated
decisions about individuals.

### Security practices

The repository excludes known credential files such as:
...
No credential, API token or password should be hard-coded in Python,
SQL, notebooks, configuration files or documentation.

Before each public release, the repository should be checked for:

* accidentally committed data files;
* local database files;
* notebook outputs containing individual rows;
...
The repository author is responsible for documenting the technical
workflow and its limitations.

Any future institutional use would require review by the relevant:

* data controller;
* data-protection officer;
* information-security team;
* clinical registry owner;
* ethics or research-governance body, where applicable.

This portfolio documentation does not constitute legal, regulatory,
ethical or clinical approval.

## 17. Author

Doudou Mazanza Mfumu
...
clinical and public-health data management;
reproducible data preparation and validation with Python;
relational database design and analytical SQL;
data-quality monitoring and issue traceability;
automated Excel reporting;
Streamlit dashboard development;
descriptive epidemiology and survival analysis;
communication of analytical results to technical and non-technical
stakeholders.

Professional interests include clinical registry data management,
healthcare analytics, data governance, epidemiological research and
decision-support reporting.

GitHub profile:

github.com/MazanzaMfumu